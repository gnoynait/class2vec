...
void train() {
	...
	// table[c] = exp(ws * c)
	float *expprod_table = new float[class_num];
    ...
    while (round < max_round) {
		...
		clear_vec(deta_w);
		update_vec(deta_w, class_vec[class_id], 1);
		for (int c = 0; c < class_num; c++) {
			update_vec(deta_w, class_vec[c], - expprod_table[c] / s);
		}
		...
		for (int c = 0; c < class_num; c++) {
			clear_vec(deta_c);
			if (c == class_id) {
				update_vec(deta_c, ws, 1);
			}
			update_vec(deta_c, ws, - a / s);
			update_vec(class_vec[c], deta_c, alpha);
		}
    }
}
