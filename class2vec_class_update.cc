...
void train_model(FILE *train_file) {
    ...
    while (read_record(train_file, code, nodes, words, len_code, len_words )) {
        ...
        // update class vector;
        float beta = starting_beta * (1 - record_count / (float) 10000);
        ...
        //use class vec to update the class hyperplane

        for (int i = 0; i < len_code; ++i) {
            float f = 0;
            float g;
            for (int j = 0; j < vec_size; ++j) {
                f += syn1[nodes[len_code] * vec_size + i] * syn1[nodes[i] * vec_size + j];
            }
            if (f <= -MAX_EXP) continue;
            else if (f >= MAX_EXP) continue;
            else f = expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
            g = (1 - code[i] - f);
            for (int c = 0; c < vec_size; ++c) {
                syn1[nodes[i] * vec_size + c] += g * syn1[nodes[len_code] * vec_size + c];
            }
        }
        ++record_count;
    }
